...
describe("TrajectoryMatcher Engine Tests", () => {
  describe("Greedy Sequence Alignment for Ordered Recommendations", () => {
    it("should match ordered recommendations strictly sequentially", () => {
      ...
      expect(resA.verdict).toBe("causal_success");
      ...
      expect(resB.verdict).toBe("non_adoption");
    ...
    it("should handle greedy pointer advancement properly", () => {
      ...
      expect(res.verdict).toBe("causal_success");
  ...
  describe("Avoid Step Violations & Causal Harm vs Contra-Adoption", () => {
    ...
    it("should identify avoid step violation on successful run as contra_adoption", () => {
      const events: ToolEvent[] = [
        { event_id: "e1", tool_name: "run_command", input_summary: "npm run dev", status: "success", started_at: "2026-05-20" }
      ];

      const res = TrajectoryMatcher.match(compiled, events, "success");
      expect(res.verdict).toBe("contra_adoption");
      expect(res.confidence).toBe("medium");
      expect(res.violatedExpectationIds.length).toBe(1);
      expect(res.evidenceRefs).toContain("e1");
    });

    it("should identify avoid step violation on failed run as contra_adoption (medium) if NO direct causal harm evidence is found", () => {
      const events: ToolEvent[] = [
        { event_id: "e1", tool_name: "run_command", input_summary: "npm run dev", status: "success", started_at: "2026-05-20" },
    ...
    it("should identify causal_harm (high) if the avoid event itself failed", () => {
      const events: ToolEvent[] = [
      ...
      const res = TrajectoryMatcher.match(compiled, events, "failure");
      expect(res.verdict).toBe("causal_harm");
      expect(res.confidence).toBe("high");
    });

    it("should identify causal_harm (high) if a failed command in the timeline contains the avoided action's keywords", () => {
      const events: ToolEvent[] = [
        { event_id: "e1", tool_name: "run_command", input_summary: "npm run dev", status: "success", started_at: "2026-05-20" },
  ...
  describe("Global Verdict Decisions and Confidence Levels", () => {
    it("should return non_adoption with low confidence when no recommended steps were matched", () => {
      const compiled = TrajectoryCompiler.compileNodeExpectations(["pnpm test"], []);
      const events: ToolEvent[] = [
      ...
      const res = TrajectoryMatcher.match(compiled, events, "success");
      expect(res.verdict).toBe("non_adoption");
      expect(res.confidence).toBe("low");
    });

    it("should return adoption with medium confidence when recommends met but outcome is failure", () => {
      const compiled = TrajectoryCompiler.compileNodeExpectations(["pnpm test"], []);
      const events: ToolEvent[] = [
      ...
      const res = TrajectoryMatcher.match(compiled, events, "failure");
      expect(res.verdict).toBe("adoption");
      expect(res.confidence).toBe("medium");
    });
    ...
    it("should handle purely avoid expectations successfully", () => {
      ...
      // Case A: No avoid action was taken, outcome is success -> causal_success
      const eventsA: ToolEvent[] = [
      ...
      // Case B: No avoid action was taken, outcome is failure -> adoption
      const resB = TrajectoryMatcher.match(compiled, eventsA, "failure");
      expect(resB.verdict).toBe("adoption");
