...
Implementation status: S1-S8 implementation and acceptance gates complete; exact published npm/ClawHub acceptance passed; the repeated S8 single-scenario campaign passed its sealed publication gate with explicit limitations
Owner: ExperienceEngine

Repeated evidence record: [OpenClaw Matched-Block Campaign V4](./openclaw-matched-block-campaign-v4.md)

> Implementation update, `2026-07-16`: this document remains the frozen design and protocol history. S1-S7 are implemented and the exact npm/ClawHub `0.5.1` artifacts passed published live-host validation. S8 implements the exhaustive contracts, dedicated append-only campaign store, sealed three-arm harness, failure/disposition/replacement protocol, complete-block scoring, publication gate, and separately labeled `openclaw-matched-block` CLI. The retained v3 one-block pilot remains immutable and correctly `not_publishable`. A new v4 real OpenClaw campaign then sealed and completed five matched blocks: all `75` preflight records passed, all `15` formal attempts terminalized at revision two, all five block dispositions are `complete`, treatment delivered in `5/5` blocks, forced holdout retained the inject decision with zero delivery in `5/5`, and no-EE remained uncontaminated in `5/5`. Independent validation and the public CLI with explicit negative-result disclosure reproduced the same scorecard digest and `publishable` campaign decision. This result passes the predeclared single-scenario campaign thresholds, but it has one scenario cluster and unavailable confidence-interval bounds, so it is directional single-scenario evidence rather than a general efficacy or support claim. `support_claim_allowed=false` and `production_learning_ready=false` remain unchanged. Historical sequencing language below records the constraints that applied at freeze time, not current task status.
...
Phase 0.5A.1 S1-S8 implementation and acceptance are complete, including exact npm/ClawHub publication acceptance. The retained v3 one-block matched three-arm pilot remains immutable and `not_publishable`. The later v4 campaign used a new sealed five-block stratum and passed the predeclared single-scenario repetition, coverage, reliability, disclosure, and quality thresholds. Runtime support remains declared only at the level proven by completed gates: v4 has one scenario cluster with unavailable confidence-interval bounds, so unrestricted production-learning/full-support claims remain blocked and `support_claim_allowed=false` plus `production_learning_ready=false` remain authoritative.
