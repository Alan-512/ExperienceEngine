...
| `cancel_package_transition` from pre-identity upgrade/rollback | require the still-selected active generation to remain schema/protocol compatible; preserve active and previous; clear pending/transition/deadline/blocked fields; set `active` | terminalize current unconsumed authorization if needed; clear current transition authorization and handshake projections; retain history | a fresh transition supervisor first drains/releases its worker, then atomically terminalizes its own lease/attempt together with this package cancellation and exits; without a fresh supervisor, gateway writer requires prior lease/attempt terminal; active runtime may subsequently use ordinary active restart |
...
No other `blocked` exit is legal. In particular, `post_identity` cannot transition directly to `active`, and pre-identity upgrade/rollback cancellation cannot change the selected active generation. Every retry uses a new activation deadline and fresh handshake identity; no prior complete or failed handshake becomes current again.
