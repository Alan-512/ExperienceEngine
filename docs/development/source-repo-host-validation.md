...
| Antigravity Agent Desktop | Windows desktop app | `ee install antigravity` in an isolated temp project, direct MCP stdio smoke from the generated `.mcp.json` command, and real Agent Desktop task execution from `C:\Users\123\Desktop\Antigravity.lnk` | install/doctor reports `host_native_hooks_validated`, `mcp_registered=true`, `hooks_registered=true`; direct stdio smoke lists ExperienceEngine MCP tools; Agent Desktop called `experienceengine_get_capabilities`; real hooks fired `PreInvocation`, `PreToolUse`, `PostToolUse`, and `Stop`; `PreToolUse` accepted `{ "decision": "allow" }`; `ee inspect --last` showed a task run for session `4efe6789-192c-43a3-863d-e75955ff2345` |
| Antigravity CLI (`agy`) | Windows CLI | `agy --add-dir <temp-project> --print --dangerously-skip-permissions --print-timeout 5m "<prompt>"` after `ee install antigravity` in an isolated temp project | CLI loads project `.agents/hooks.json`; real hooks fired `PreInvocation`, `PreToolUse`, `PostToolUse`, and `Stop`; `PreToolUse` accepted `{ "decision": "allow" }`; `ee inspect --last` showed task runs for session `c379ba97-c907-449a-a4d1-fee58dad0db7` |
...
- Global Antigravity hooks supported: no verified supported surface. `agy config --help` and `agy hooks --help` currently fall back to top-level help and do not expose a global MCP/hooks config command. Project wiring remains required for Agent Desktop, while `ee agy exec -C <project>` can auto-bootstrap project wiring for CLI runs.
