...
| Antigravity IDE on Windows | MCP cache observed; hooks not validated | Local inspection found EE MCP tool cache files under `~/.gemini/antigravity-ide/mcp/experienceengine`, but no IDE EE hook plugin under `~/.gemini/antigravity-ide/plugins/experienceengine`. Treat IDE lifecycle automation as not adapted until a real IDE hook validation passes. |
