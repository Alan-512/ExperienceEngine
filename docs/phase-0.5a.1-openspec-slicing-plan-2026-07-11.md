...
- Historical review approval originally permitted implementation to begin with S1 only. Current progress is recorded in the status table above: S1-S8 implementation gates are complete. The retained v3 one-block pilot did not meet the five-repetition threshold and remains `not_publishable`; the later separately sealed v4 five-block campaign passed its single-scenario gate. Its one-cluster limitation prevents a general efficacy/full-support claim, so `support_claim_allowed=false` and `production_learning_ready=false` remain required.
