...
export const renderCodexExperienceEngineInstruction = (): string =>
  [
    ...
    "- Use `experienceengine_record_tool_result` only for important tool outcomes that changed the task direction.",
    "- Call `experienceengine_finalize_task` before finishing each meaningful task so ExperienceEngine can persist the outcome.",
