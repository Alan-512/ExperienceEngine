...
export const renderCodexExperienceEngineInstruction = (): string =>
  [
    ...
    "- If injected guidance clearly helped or harmed the task, mark the guidance as helped or harmed in-session with `experienceengine_quick_feedback` before CLI fallback.",
    ...
    "- Call `experienceengine_finalize_task` before finishing. If the task did not materially change after lookup, omit `prompt` and send only a concise `contextSummary` when useful."
  ].join("\n");
