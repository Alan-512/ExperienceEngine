...
type RegistryDeps = {
  ...
  operationalSurface: {
    checkUpdate: (adapter: ExperienceAdapter) => Promise<unknown>;
  };
  operationalActions: {
    planOperation: (args: {
  ...
  const actions: CodexActionDefinition[] = [
    {
      ...
      handler: async ({ adapter }) =>
        deps.operationalActions.planOperation({
          adapter: adapter as ExperienceAdapter,
          operation: "install"
        })
    },
    {
    ...
    },
    {
      ...
      examplePayload: { adapter: "codex" },
      handler: async ({ adapter }) =>
        deps.operationalSurface.checkUpdate(adapter as ExperienceAdapter)
    },
    {
