...
export type SharedLookupArgs = {
  cwd?: string;
  prompt: string;
  sessionId?: string;
};

export type SharedToolResultArgs = {
  sessionId: string;
  toolName: string;
  inputSummary?: string;
...
export type SharedFinalizeArgs = {
  sessionId: string;
  cwd?: string;
  prompt?: string;
...
export type SharedHostEventArgs = {
  cwd?: string;
  sessionId?: string;
...
const createSharedConfig = (adapter: ExperienceAdapter, options: SharedServerOptions = {}) => {
...
const createSharedPromptRuntime = (adapter: ExperienceAdapter, options: SharedServerOptions = {}): ExperiencePromptRuntimeService =>
  new ExperiencePromptRuntimeService(createSharedConfig(adapter, options));

const createSharedRuntime = async (adapter: ExperienceAdapter, options: SharedServerOptions = {}): Promise<ExperienceRuntimeService> => {
  const { ExperienceRuntimeService } = await import("../../runtime/service.js");
  return new ExperienceRuntimeService(createSharedConfig(adapter, options), undefined, {
...
export const createSharedBehaviorLoop = (adapter: ExperienceAdapter, options: SharedServerOptions = {}) => {
  ...
  const getPromptRuntime = () => {
    promptRuntime ??= createSharedPromptRuntime(adapter, options);
    return promptRuntime;
  };

  const getRuntime = async () => {
    runtime ??= createSharedRuntime(adapter, options);
    return runtime;
  };

  return {
    async lookupHints(args: SharedLookupArgs) {
      if (args.sessionId) {
    ...
    async signalHostEvent(args: SharedHostEventArgs) {
      const fullRuntime = await getRuntime();
    ...
    async recordToolResult(args: SharedToolResultArgs) {
      const fullRuntime = await getRuntime();
    ...
    async finalizeTask(args: SharedFinalizeArgs) {
      const fullRuntime = await getRuntime();
