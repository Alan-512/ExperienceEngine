...
export type SharedMcpServerOptions = SharedServerOptions & {
...
type SharedRecentMode = "all" | "injected";
type SharedScopeArgs = {
  cwd?: string;
};
...
const EXPERIENCE_ADAPTERS = ["openclaw", "claude-code", "codex", "antigravity"] as const;

const createSharedConfig = (adapter: ExperienceAdapter, options: SharedMcpServerOptions = {}) => {
...
const createSharedInteractionService = (
  adapter: ExperienceAdapter,
  options: SharedMcpServerOptions = {}
): ExperienceInteractionService => {
  return new ExperienceInteractionService(createSharedConfig(adapter, options));
};

const createSharedOperationalService = (
  options: SharedMcpServerOptions = {}
): ExperienceOperationalService => {
...
export const createSharedInteractionSurface = (adapter: ExperienceAdapter, options: SharedMcpServerOptions = {}) => {
  ...
  return {
    ...
    async inspectRecent(args: { mode?: SharedRecentMode; limit?: number } = {}) {
    ...
    async inspectRepoSummary(args: SharedScopeArgs = {}) {
      return interaction.inspectRepoSummary(args.cwd);
    },

    async inspectReview(args: SharedScopeArgs & { limit?: number } = {}) {
      return interaction.inspectReview(args.cwd, { limit: args.limit });
    },

    async inspectGovernance(args: SharedScopeArgs = {}) {
      return interaction.inspectGovernance(args.cwd);
    },

    async inspectGovernancePendingApprovals(args: SharedScopeArgs = {}) {
    ...
    async disableScope(args: SharedScopeArgs = {}) {
      return interaction.disableScope(args.cwd);
    },

    async enableScope(args: SharedScopeArgs = {}) {
...
export const createSharedMcpServer = (adapter: ExperienceAdapter, options: SharedMcpServerOptions = {}) => {
