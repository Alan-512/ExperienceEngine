...
const buildNodeReasons = (input: ExperienceInput, node: ExperienceNode): string[] => {
  ...
  if (resolveEffectiveNodeDeliveryState(node) === "conservative_only") {
    reasons.push("This node is limited to conservative delivery in live tasks.");
  } else if (resolveEffectiveNodeDeliveryState(node) === "shadow_only") {
    reasons.push("This node is restricted to shadow evaluation and should not inject live.");
  } else if (resolveEffectiveNodeDeliveryState(node) === "quarantined") {
...
export const buildInjectionScorecard = (
  ...
  nodes: ExperienceNode[],
  sessionId?: string,
  diagnostics?: Partial<InterventionDecisionDiagnostics>
): InjectionScorecard => {
  ...
  return {
    ...
    secondOpinionApplied: diagnostics?.secondOpinionApplied,
    secondOpinionDecision: diagnostics?.secondOpinionDecision,
