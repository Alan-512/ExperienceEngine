...
ExperienceEngine SHALL store append-only attribution records that link an intervention outcome to an individual experience node.
...
- **AND** each record includes `injection_id`, `node_id`, `intervention_strength`, `delivered=true`, outcome signal, attribution verdict, confidence, evidence references, and creation time
