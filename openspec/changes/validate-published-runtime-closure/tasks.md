...
- On `2026-07-15`, the exact candidate with SHA-256 `07a93076c1ab196ffd790bb8c53885899ee33aca8415e4d5ed3242d082abac00` was extracted into the fresh `.tmp/clawhub-candidate-v0.5.0-4a0bced-final/package` staging root. ClawHub CLI `0.23.1` Plugin Inspector returned `pass` with `0` breakages, warnings, deprecations, or issues. The report was written outside the package root so the inspected staging bytes remained uncontaminated. The inspector did not receive a separate OpenClaw checkout (`targetOpenClaw.status=disabled`); host compatibility remains covered by the independent real-host gate.
- A no-upload dry-run against the exact `.tgz`, with explicit `code-plugin` family and source provenance `Alan-512/ExperienceEngine@4a0bced`, resolved `@alan512/experienceengine@0.5.0` with exactly `1135` files and `1133532` bytes. No npm or ClawHub publication occurred.
- Initial WSL retries used Ubuntu-22.04, Node.js `24.18.0`, and OpenClaw `2026.7.1 (2d2ddc4)`. They repeatedly reached native activation replay before external model routing failed: OpenRouter free routes were rate-limited, the configured StepFun route no longer existed, and the configured Google route had no API key. Every failed attempt completed host and validation-root cleanup.
- The real WSL default was then changed to the current HY3 free model. OpenClaw proved that the unqualified catalog id `tencent/hy3:free` incorrectly selects a nonexistent standalone `tencent` provider, so the final persisted host id is the provider-qualified `openrouter/tencent/hy3:free`; the invalid unqualified allowlist entry was removed. The systemd user Gateway on port `18789` is running OpenClaw `2026.7.1`, and its connectivity probe passes.
- With `openrouter/tencent/hy3:free`, the complete local-pack real-host gate passed. It observed the real agent turn, Gateway restart recovery, plugin/runtime readiness, read-only activation preparation, activation initialization and idempotent replay, semantic queue completion, stale-authority completion rejection, interruption recovery, and graceful worker/supervisor terminal authority. The local-pack artifact integrity was `sha256:d129b8c085f2cc20fb9c10520fdf0f372406760c9476650d9f79724729386e98`; runtime closure digest remained `06bad602d9d06909035aa2cc6052d3daca06cf22c76d5d53c47b5d34ef933f63`.
- Published acceptance tasks 2.1-2.5, 5.2, and 5.3 remain unchecked. This successful host rerun is local prepublication evidence only; `production_learning_ready`, `artifact_runtime_validated`, and `support_claim_allowed` remain false.
...
- S8 matched-block implementation and acceptance are complete. The retained v3 one-block pilot remains immutable and `not_publishable`; the later separately sealed v4 campaign completed five full blocks and passed its single-scenario publication thresholds. Because v4 contains one scenario cluster with unavailable confidence-interval bounds, `production_learning_ready=false` and `support_claim_allowed=false` remain mandatory and no general full-support claim is authorized.
