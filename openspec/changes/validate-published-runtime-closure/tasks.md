...
- A fresh WSL retry used Ubuntu-22.04, Node.js `24.18.0`, and OpenClaw `2026.7.1 (2d2ddc4)`. It passed artifact materialization, seed-auth migration, OpenClaw version resolution, host security scan, isolated installation, installed-package verification, runtime-authority preparation, Gateway readiness, plugin load, runtime service readiness, native activation initialization, and idempotent activation replay. The subsequent real agent turn was externally blocked by OpenRouter rate limiting, after which both host cleanup and validation-root cleanup completed.
- Safe isolated route checks did not change the real WSL configuration: the configured StepFun free model no longer exists at the provider, a current tool-capable OpenRouter free model hit the same account/provider rate limit, and the configured Google route has no API key. These retries therefore do not constitute a new full real-host pass and must not be represented as one.
- Published acceptance tasks 2.1-2.5, 5.2, and 5.3 remain unchecked. This evidence is local prepublication evidence only; `production_learning_ready`, `artifact_runtime_validated`, and `support_claim_allowed` remain false.
