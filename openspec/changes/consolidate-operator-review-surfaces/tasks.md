...
- [ ] 2.1 Add `ExperienceInteractionService.inspectReview()` to resolve scope, collect existing repo/hygiene/export-draft reports, and return the operator review report without mutating stored state.
- [ ] 2.2 Add `ee inspect review` with scope/cwd and limit filters.
- [ ] 2.3 Add a Codex/MCP `experienceengine://review` read-only resource and broker action `inspect_operator_review`.
- [ ] 2.4 Update MCP capabilities metadata to list the review resource.
...
- [ ] 5.1 Run targeted unit tests for operator review flow, interaction service, CLI, and Codex/MCP surfaces.
- [ ] 5.2 Run `pnpm check`.
- [ ] 5.3 Run `openspec validate consolidate-operator-review-surfaces --strict` and `openspec validate --changes --strict`.
