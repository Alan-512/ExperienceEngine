...
#### Scenario: Warning candidates converge on one canonical node key
- **GIVEN** repeated failures for the same scope and task family arise from different tools
- **WHEN** ExperienceEngine stores warning candidates for those failures
- **THEN** the warning node key remains stable across the different failure sources
- **AND** tool-specific detail stays in non-key metadata instead of fragmenting warning node ids
