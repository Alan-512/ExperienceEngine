...
- [x] 4.9 Inspect Antigravity IDE as a separate product surface. *(Local IDE state uses `~/.gemini/antigravity-ide`; EE MCP tool cache files were observed under `~/.gemini/antigravity-ide/mcp/experienceengine`, but no EE IDE hook plugin was observed. `ee doctor antigravity` now reports IDE MCP cache and IDE hook observation separately.)*
